...
norm_preserve_deform_conv_cuda_forward(const at::Tensor &input,
                    ...
                    const bool zero_padding)
{
    ...
    AT_ASSERTM(batch % im2col_step_ == 0, "forward batch(%d) must divide im2col_step(%d)", batch, "   ", im2col_step_);
    ...
    auto input_squared = at::square(input);
    //std::cout << "forward input_squared:" << std::endl;
    //std::cout << input_squared << std::endl;

    auto columns = at::empty({batch/im2col_step_, channels * kernel_h * kernel_w, batch_n * height_out * width_out}, input.options());
    for (int n = 0; n < batch/im2col_step_; ++n)
    {
        auto columns_n = columns.select(0, n).view({channels * kernel_h * kernel_w, batch_n * height_out * width_out});
        AT_DISPATCH_FLOATING_TYPES(input_squared.type(), "deform_conv_forward_cuda", ([&] {
            deformable_im2col_cuda(at::cuda::getCurrentCUDAStream(),
                                             input_squared.data<scalar_t>() + n * im2col_step_ * per_input_size,
        ...
        auto sqrt_columns_n = at::sqrt(columns_n);

        auto sqrt_columns_g = sqrt_columns_n.view({group, channels/group * kernel_h * kernel_w, batch_n * height_out * width_out});

        auto output_g = output_n.select(0, n).view({batch_n * height_out * width_out, group, channels_out/group});
        for (int g = 0; g < group; ++g)
        {
            auto sqrt_columns_gm = sqrt_columns_g.select(0, g).t();
            auto weight_gm = weight_g.select(0, g).view({channels_out/group, channels_kernel * kernel_h * kernel_w}).t();
            auto output_m = at::addmm(bias_g.select(0, g), sqrt_columns_gm, weight_gm);
...
std::vector<at::Tensor> norm_preserve_deform_conv_cuda_backward(const at::Tensor &input,
                                             const at::Tensor &deformed_columns,
                                             const at::Tensor &weight,
                                             const at::Tensor &bias,
                                             const at::Tensor &offset,
                                             ...
                                             const bool zero_padding)
{
    ...
    AT_ASSERTM(batch % im2col_step_ == 0, "backward batch(%d) must divide im2col_step(%d)", batch,  "   ", im2col_step_);
    ...
    for (int n = 0; n < batch/im2col_step_; ++n)
    {
        auto deformed_columns_n = deformed_columns.select(0, n).view({channels * kernel_h * kernel_w, batch_n * height_out * width_out});
        ...
        columns = (1 / (2 * at::sqrt(deformed_columns_n) + epsilon)) * columns;
        columns_g = columns.view({group, channels/group * kernel_h * kernel_w, batch_n * height_out * width_out});
