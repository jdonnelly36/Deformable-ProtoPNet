...
if "stanford_dogs" in load_model_path:
    train_push_dir = './datasets/stanford_dogs/train/'
...
def save_prototype_box(fname, epoch, index):
    try:
        ...
        plt.imsave(os.path.join(fname, 'prototype-img-with_box'+str(index)+'.png'), p_img)
    except:
...
def save_def_prototype_patches(fname, epoch, index):
    for j in range(9):
        try:
            p_img = plt.imread(os.path.join(load_img_dir, 'epoch-'+str(epoch), 'prototype-img'+str(index)+'_patch_'+str(j)+'-with_box.png'))
            plt.imsave(os.path.join(fname, 'prototype-img'+str(index)+'_patch_'+str(j)+'.png'), p_img)
        except:
